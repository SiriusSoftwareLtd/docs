...
```lua
local farm = tab:CreateToggle({ name = "Auto Farm", description = "Collects while you idle." })

farm:Lock("Needs a premium key")  -- the reason replaces the description
farm:Unlock()                     -- and the description comes back
```

<ResponseField name="Lock(reason?)">
  Lock it. The reason takes the place of the description while it's locked; without one, the description stays as it was.
</ResponseField>

<ResponseField name="Unlock()">
  Unlock it.
</ResponseField>
...
<CardGroup cols={2}>
  ...
  <Card title="Progress" icon="loader" href="/rayfield-gen2/elements/progress">
    How far along something is.
  </Card>
  <Card title="Console" icon="terminal" href="/rayfield-gen2/elements/console">
    Live log output, or a block of code.
  </Card>
  <Card title="Text" icon="type" href="/rayfield-gen2/elements/text">
    A title, a body, or both.
  </Card>
  <Card title="Divider" icon="minus" href="/rayfield-gen2/elements/dividers">
