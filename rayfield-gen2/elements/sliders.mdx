...
On the preview channel the callback takes a second argument: `(value, dragging)`. `dragging` is true from the moment the handle is grabbed until it is let go, with a final call of false once it settles, so you can show a preview while it moves and commit when it stops.
